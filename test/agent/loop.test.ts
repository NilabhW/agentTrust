...
function fakeGeminiClient(responses: GenerateContentResult[]) {
  let call = 0;
  return {
...
describe("runBuyerAgent", () => {
  it("stops with goal_complete as soon as the model responds with no function call", async () => {
    const geminiClient = fakeGeminiClient([{ status: "success", text: "All done, bought the rice.", functionCalls: [] }]);

    const transcript = await runBuyerAgent({ ...baseOpts, geminiClient });
  ...
  it("executes a tool call, feeds the result back, and continues the loop", async () => {
    const geminiClient = fakeGeminiClient([
      {
        status: "success",
        text: null,
    ...
    const secondCallInput = geminiClient.generateContent.mock.calls[1][0];
  ...
  it("stops at the turn limit rather than looping forever", async () => {
    ...
    const transcript = await runBuyerAgent({ ...baseOpts, geminiClient, turnLimit: 3 });

    expect(transcript.stopReason).toBe("turn_limit_reached");
    expect(transcript.turns).toHaveLength(3);
    expect(geminiClient.generateContent).toHaveBeenCalledTimes(3);
  });
  ...
  it("stops gracefully (does not throw) when the Gemini call itself fails", async () => {
    const geminiClient = fakeGeminiClient([{ status: "failed", raw_error: "API key not valid" }]);
  ...
  it("passes the tool declarations and a system instruction mentioning the agent's allowed categories", async () => {
    const geminiClient = fakeGeminiClient([{ status: "success", text: "done", functionCalls: [] }]);

    await runBuyerAgent({ ...baseOpts, geminiClient });

    const input = geminiClient.generateContent.mock.calls[0][0];
